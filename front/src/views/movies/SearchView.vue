<!-- <template>
  <div class="search">
    ...
    <div v-if="sentiment" class="result-section">
      <h2>감정 분석 결과:</h2>
      <p>감정 점수: {{ sentiment }}</p>
    </div>
...
onMounted(() => {
  ...
  transcript.value = route.params.transcript || ''
  sentiment.value = route.params.sentiment || null
  movies.value = route.params.movies ? JSON.parse(route.params.movies) : []
})
</script>
